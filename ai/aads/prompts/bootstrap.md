...
2. **Autoridade / conflitos**  
   `engine/source-of-truth-map.md`

3. **Execução (máquina de estados + runtime)**  
   `engine/aads-operating-model.md`  
   `engine/execution-engine.md`  
   `engine/developer-protection.md`

4. **Constituição** (limites permanentes)  
   `constitution/`

5. **Demais camadas** conforme o mapa e a classificação do trabalho  
